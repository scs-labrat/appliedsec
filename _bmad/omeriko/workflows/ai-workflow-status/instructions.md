# AI Workflow Status Check - Multi-Mode Service (Omeriko)

<critical>The workflow execution engine is governed by: {project-root}/_bmad/core/tasks/workflow.xml</critical>
<critical>You MUST have already loaded and processed: {project-root}/_bmad/omeriko/workflows/ai-workflow-status/workflow.yaml</critical>
...
<action>Search {output_folder}/ for file: ai-workflow-status.yaml</action>

<check if="no status file found">
  <output>No AI workflow status found.</output>
  <ask>Would you like to run AI Workflow Init now? (y/n)</ask>

  <check if="response == y OR response == yes">
    <action>Launching workflow-init to set up your AI project tracking...</action>
    <invoke-workflow path="{project-root}/_bmad/omeriko/workflows/ai-workflow-status/init/workflow.yaml"></invoke-workflow>
...
<action>Read ai-workflow-status.yaml</action>
...
- ai_system_type (from YAML field)
...
<output>
## 🤖 AI System Dev Status

**Project:** {{project}} ({{ai_system_type}} - {{field_type}})
...
- {{workflow_name}} ({{agent}} {{command}}): {{status_display}}
  {{/each}}
  {{/each}}
...
{{#if in_omeriko_phase}}
Run: `{{next_command}}` with Omeriko
{{/if}}

{{#if in_bmm_phase}}
Switch to BMM agent: `{{next_agent}}`
{{/if}}
...
{{#if next_agent == omeriko}}
Continue with Omeriko: Run `{{next_command}}`
{{else}}
Switch to {{next_agent}} agent for this step
{{/if}}
</output>
</check>
...
<check if="choice == 3">
  <action>Display complete ai-workflow-status.yaml file contents</action>
</check>
...
Your choice:</ask>
  ...
  <check if="update_choice == 1">
    <ask>Which workflow? (Enter workflow ID like 'system-design' or 'knowledge-base')</ask>
    <ask>File path created? (e.g., docs/ai-system-design.md)</ask>
...
<action>Read {output_folder}/ai-workflow-status.yaml if exists</action>
...
<template-output>ai_system_type = {{ai_system_type}}</template-output>
...
<template-output>status_file_path = {{path to ai-workflow-status.yaml}}</template-output>
</check>
...
<action>Check if {output_folder}/ai-workflow-status.yaml exists</action>
...
<action>Read {output_folder}/ai-workflow-status.yaml</action>
